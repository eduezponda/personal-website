...
@coding_guille's point is valid but incomplete. The embedding pipeline is overused when the dataset is small and the search is structured. For everything else, the complexity is warranted. The mistake is treating either approach as a universal answer.
